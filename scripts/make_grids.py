...
for type in grid_types:
    mapgrid = MapGrid(type, args.name, float(args.resolution), float(args.lat_min),
            float(args.lat_max), float(args.lon_min), float(args.lon_max),
            int(args.num_solutions), float(args.nyquist_correction), float(args.mu),
            float(args.qconst), float(args.beta), pickle_path)
